...
class RedCapEtl
{
    ...
    public function autoGenerateRules($addFormCompleteField = false, $addDagField = false)
    {
        ...
        # Get config values
        $configAddCompleteFields = false;
        $configAddDagFields = false;
        if ($this->configuration->getAutoGenerateCompleteFields()) {
            $configAddCompleteFields = true;
        }
        if ($this->configuration->getAutoGenerateDagFields()) {
            $configAddDagFields = true;
        }

        # Set flags if either parameter or config value is set for flag
        $addFormCompleteField = $addFormCompleteField || $configAddCompleteFields;
        $addFormDagField      = $addDagField || $configAddDagFields;
