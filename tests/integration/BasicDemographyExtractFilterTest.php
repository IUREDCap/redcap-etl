...
class BasicDemographyExtractFilterTest extends TestCase
{
    ...
    const CONFIG_FILE = __DIR__.'/../config/basic-demography-extract-filter.ini';
    ...
    public function testDemographyTable()
    {
        ...
        $expectedCsv = CsvUtil::csvFileToArray(__DIR__.'/../data/BasicDemographyExtractFilter.csv');

        $header = $csv[0];
        $this->assertEquals($header[2], 'record_id', 'Record id header test.');

        // 1 header row + 7 data rows (8 total rows) expected
        $this->assertEquals(8, count($csv), 'Demography row count check.');
        ...
        $expectedCsv = CsvUtil::csvFileToArray(__DIR__.'/../data/BasicDemographyExtractFilterLabelView.csv');
